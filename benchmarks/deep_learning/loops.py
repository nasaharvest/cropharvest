...
def train(
    ...
    if sample_size is not None:
        train_batch_total = dataset.sample_train(sample_size // 2)
    ...
    for i in tqdm(range(num_grad_steps)):
        ...
        if sample_size is not None:
            ...
            indices, state = sample_with_memory(train_batch_total, k * 2, state)
            train_x, train_y = train_batch_total[0][indices], train_batch_total[1][indices]
        else:
            train_x, train_y = dataset.sample_train(k, deterministic=False)
